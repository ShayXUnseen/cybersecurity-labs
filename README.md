# cybersecurity-labs
Today I learned various Linux terminal commands, as demonstrated in Lab 002 - Basic Terminal Commands.

Let me give you an overview. The `ls` commands list the directories present. The `cd` command changes the directory, while `cd ..` changes the directory one step up. The `mkdir` command creates a directory. `nano` is an editor, so `nano` followed by a file name opens or creates that file.

The `cp` command copies a file, while `rm` deletes a file. The `mv` command is used to move as well as rename a file.

A special flag, `-r`, is used with the copy and remove commands to copy or remove a directory along with all its contents. `-r` stands for **recursive**.

Here's the proof of my progress :
<img width="1212" height="555" alt="image" src="https://github.com/user-attachments/assets/a4d24aed-cd00-49d1-ad8f-9a445ae3e670" />

